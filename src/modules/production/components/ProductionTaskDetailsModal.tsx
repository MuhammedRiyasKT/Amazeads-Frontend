...
import { getProductionTaskDetails } from "../services/productionTask.service";
...
export default function ProductionTaskDetailsModal({ isOpen, taskId, onClose }: ProductionTaskDetailsModalProps) {
  ...
  useEffect(() => {
    if (isOpen && taskId) {
      setIsLoading(true);
      getProductionTaskDetails(taskId)
        .then(setDetails)
        .catch(console.error)
      ...
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-2xl overflow-hidden border border-slate-100">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b bg-slate-50/50">
          <div className="flex items-center gap-2">
            ...
            <h3 className="font-bold text-slate-800 text-sm uppercase">Production Specifications & Files</h3>
          </div>
        ...
        {/* Content */}
        {isLoading ? (
          <div className="p-10 text-center text-xs text-slate-500 font-semibold">Loading production specifications...</div>
        ) : !details ? (
          ...
          <div className="p-6 flex flex-col gap-5 text-xs font-semibold text-slate-600 max-h-[80vh] overflow-y-auto">
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-3 bg-slate-50 p-4 rounded-lg border border-slate-100">
              ...
              <div>
                <span className="text-[10px] text-slate-400 uppercase block">Designing Status</span>
            ...
            {/* Images Section */}
            <div>
              <h4 className="text-[11px] font-bold text-slate-400 uppercase mb-2">Production Artwork Images</h4>
