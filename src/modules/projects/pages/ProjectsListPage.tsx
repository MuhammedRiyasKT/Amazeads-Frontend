...
import { Eye, Plus, Filter, RotateCcw, Calendar, User } from "lucide-react";
import Pagination from "@/components/ui/Pagination";
import { getAllPMProjects, UserRole } from "@/modules/project-manager/services/managerOrder.service";
...
import ProjectDeptStatusModal from "@/modules/project-manager/components/ProjectDeptStatusModal";
...
export function ProjectsListPage({ role = "project-manager" }: { role?: UserRole }) {
                      ...
                      {projectsList.map((proj: any, pIdx: number) => {
                        ...
                        return (
                          <tr key={`${order.order_id || order.id}-${proj?.id || pIdx}`}>
                            ...
                            {isFirstRow && (
                              <td rowSpan={projectsCount} style={{ textAlign: "center" }} className="align-middle">
                                <span className={`px-2.5 py-0.5 text-[10px] font-bold rounded-md border whitespace-nowrap inline-block ${order.order_status === "Delivered" ? "bg-emerald-50 text-emerald-700 border-emerald-200" :
