...
export default function HRLeavePage() {
  ...
  useEffect(() => {
    getHRStaffs().then((data) => setStaffsList(data || [])).catch((err) => console.error(err));
    getHRRoles().then((data) => setDepartments(data.filter((r) => r.role_name.toLowerCase() !== "admin"))).catch((err) => console.error(err));
  }, []);
  ...
  const handleApprove = async (id: number) => {
    if (!window.confirm("Approve this leave request as HR?")) return;
    try {
      await approveLeaveByHR(id, currentHRId);
  ...
  const handleReject = async (id: number) => {
    if (!window.confirm("Reject this leave request as HR?")) return;
    try {
      await rejectLeaveByHR(id, currentHRId);
  ...
  const hrApprovedCount = leaves.filter((l) => l.status === "HR Approved").length;
      ...
      <div className="bg-white border rounded-xl overflow-hidden shadow-sm">
        ...
        {pagination.total_pages > 1 && (
          <div className="flex justify-between items-center bg-white border-t px-5 py-4 shadow-sm">
            <div className="text-xs text-slate-500">Showing page {pagination.page} of {pagination.total_pages}</div>
