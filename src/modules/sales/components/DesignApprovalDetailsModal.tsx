...
import { X, User, Layout, ZoomIn, Calendar } from "lucide-react";
import Button from "@/components/ui/Button";
import { getSalesProjectDetails } from "../services/designApproval.service";

interface DesignApprovalDetailsModalProps {
  isOpen: boolean;
  taskId: number | null;
  onClose: () => void;
}

export default function DesignApprovalDetailsModal({ isOpen, taskId, onClose }: DesignApprovalDetailsModalProps) {
        ...
        <div className="bg-white rounded-xl shadow-2xl w-full max-w-2xl overflow-hidden border">
          ...
          <div className="flex items-center justify-between px-6 py-4 border-b bg-slate-50/50">
            <div className="flex items-center gap-2">
              ...
              <h3 className="font-bold text-slate-800 text-sm uppercase">Artwork Spec Review</h3>
            </div>
          ...
          {isLoading || !details ? (
            ...
            <div className="p-6 flex flex-col gap-5 max-h-[75vh] overflow-y-auto text-xs font-semibold text-slate-600">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 border-t pt-3">
                ...
                <div className="flex flex-col">
                  <span className="text-[10px] font-bold text-slate-400 uppercase">Design Date</span>
                  <span className="text-indigo-600 font-bold mt-0.5">{details.design_date || "—"}</span>
                </div>
                <div className="flex flex-col">
                  <span className="text-[10px] font-bold text-slate-400 uppercase">Print Date</span>
              ...
              {details.images && details.images.length > 0 && (
                <div className="flex flex-col gap-2 border-t pt-4">
                  <span className="text-[10px] font-bold text-slate-400 uppercase">Design Proof Image Files ({details.images.length})</span>
